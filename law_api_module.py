"""
법제처 Open API 연동 모듈 v1.3
========================
v1.3 수정사항:
  - 약칭 매핑: 부분 매칭 지원 ("표시광고법" → "표시광고" 자동 변환)
  - 법령 상세 조회: ID → MST 파라미터로 수정 (법제처 API 실제 스펙)
  - 해석례 상세 보기 추가
  - placeholder 텍스트에 약칭 사용 가능 명시
"""
...
class LawAPI:
    ...
    def get_law_text(self, mst: str, jo: Optional[str] = None) -> dict:
        """법령 조문 조회. MST 파라미터 사용 (법제처 API 스펙)."""
        params = {
            "OC": self.oc,
            "target": "law",
            "type": "XML",
            "MST": mst,  # ← 핵심: ID가 아니라 MST
        }
        if jo:
            params["JO"] = jo
        ...
        if not articles and not law_name:
            ...
            logger.warning(f"조문 파싱 실패. MST={mst}, 태그: {all_tags}")
    ...
    def get_interpretation_detail(self, interp_id: str) -> dict:
        ...
        if error:
            return {"error": f"API 호출 실패: {error}"}
        return {
            "안건명": tree.findtext(".//안건명", ""),
            "안건번호": tree.findtext(".//안건번호", ""),
            "회답일자": tree.findtext(".//회답일자", ""),
            "회답기관": tree.findtext(".//회답기관", ""),
            "질의요지": tree.findtext(".//질의요지", ""),
            "회답": tree.findtext(".//회답", ""),
            "이유": tree.findtext(".//이유", ""),
...
def render_law_search_sidebar():
    with st.sidebar:
        ...
        law_query = st.text_input(
            "법령 검색어",
            placeholder="예: 표시광고법, 대규모유통업법, 관세법",
        ...
        if st.button("🔍 검색", key="law_search_btn") and law_query:
            try:
                ...
                st.session_state["law_search_results"] = {
                    "query": law_query,
                    "resolved": _resolve_keyword(law_query),
                        ...
                        if st.button(f"📄 조문 전문 보기", key=f"load_law_{i}"):
                            try:
                                ...
                                if "error" not in detail and detail.get("조문목록"):
                                    ...
                                    error_msg = detail.get("error", "조문을 불러오지 못했습니다.")
                                    st.warning(f"⚠️ {error_msg}")
                                    st.caption(f"MST: {mst}")
                            except Exception as e:
                ...
                with st.expander(
                    ...
                    if interp_id:
                        ...
                        if cache_key in st.session_state:
                            ...
                            if detail.get("질의요지"):
                                st.markdown("**❓ 질의요지**")
                                st.markdown(detail["질의요지"])
                            if detail.get("회답"):
                                st.markdown("**💬 회답**")
                                st.markdown(detail["회답"])
                            if detail.get("이유"):
                                st.markdown("**📝 이유**")
                                st.markdown(detail["이유"])
                            if detail.get("참조조문"):
                                st.markdown(f"**⚖️ 참조조문:** {detail['참조조문']}")
                    else:
