...
_TOPIC_ACTION_PLAN = {
    ...
    "판촉비분담": (
        # ⑤ 충족 시 §11①②③④ 전부 적용제외 → ①② 서면약정은 의무 아님. 아래 셋은 입증자료·리스크관리.
        "· [⑤ 충족 경로] 협력사 '자발적 요청' 공문 확보\n"
        "· [⑤ 충족 경로] 차별화 입증 기획안 + 협의 분담비율 서면화 (입증자료 — §11①② 의무 아님)\n"
        # ⑤ 미충족(요건 결여) 시에만 §11①②③④ 적용 → 서면약정·동시교부 + 50% 상한.
        "· [⑤ 미충족 시] §11①②③④ 적용 — 서면약정·동시교부(§11①②)·50% 상한(§11④), 60% 요구는 ④ 위반\n"
        "· 사내변호사 확인 = '⑤ 요건(자발성·차별화)·입증 충분성'만"
    ),
}
...
def build_system_gemini_stage1(docs, laws_db):
    ...
    return (
        ...
        "██ 사규 인용 강제 규칙 (위반 시 자동 차단) ██\n\n"
        "사규를 인용할 때는 반드시 아래 허용 label 중 하나를 토씨 하나 안 바꾸고 그대로 사용한다.\n\n"
        "**현재 등록된 사규 label (이 중 하나만 인용 가능):**\n"
        "- `사규: 사규 모음.docx` 또는 `사규 모음.docx`\n"
        "- `계약서(특약매입): 특약매입.docx` 또는 `특약매입.docx`\n"
        "- `계약서(직매입): 직매입.docx` 또는 `직매입.docx`\n"
        "- `약정서(공동판촉): 공동 판촉행사 약정서.docx`\n"
        "- `약정서(매장이동): 매장 이동.docx`\n"
        "- `약정서(인테리어설치): 인테리어 설치 약정서.docx`\n"
        "- `약정서(협력사원): 협력사원 지원 약정서.docx`\n"
        "- `판매촉진비용 부담 지침` (사규 모음.docx 내부 사규)\n"
        "- 위 항목의 짧은 형태(예: `계약서(특약매입)`, `특약매입 계약서`, `공동 판촉행사 약정서`, "
        "`매장 이동 약정서`, `인테리어 설치 약정서`, `협력사원 지원 약정서`)도 모두 인용 허용\n\n"
        "**금지 행위:**\n"
        "- '반품 절차 지침' 같이 임의로 사규명 만들기 → Gatekeeper 자동 차단\n"
        "- 사규 내 조항만 인용하고 label 생략\n"
        "- '특정매입거래 기본계약서' 같은 일반론적 명칭 사용 (실제 등록 label과 다름)\n\n"
        "**올바른 예시:**\n"
        "- ✅ '계약서(특약매입): 특약매입.docx 제3조에 따르면...'\n"
        "- ❌ '반품 절차 지침에 따르면...'\n\n"
        "**검색 결과 매칭 없을 시:**\n"
        "'해당 규정 없음' 명시. 절대 추측·창작 금지.\n\n"
        ...
        "  ① 상표법 제230조 (침해죄 7년/1억) + 제235조 (양벌규정: 법인 3억) + 제236조 (필요적 몰수)\n"
        "     - 고의 요건: 면세점은 국가 특허 공식 유통채널로 정품 인증 의무 있으므로 '몰랐다' 항변 불가. 고의 입증 용이.\n"
        "     - 징벌적 손해배상: 2025.7.22 시행 개정으로 종전 3배→5배 상향\n"
        "     - 필요적 몰수(제236조): 침해물품+제작용구·재료 전량 강제 몰수 (법원 재량 아닌 의무)\n"
        "  ② 형법 제347조 (사기죄)\n"
        "     - 2025.12.23 개정: 종전 10년 이하 징역/2천만원 이하 벌금 → 20년 이하 징역/5천만원 이하 벌금으로 2배 상향\n"
        "     - 상표법 위반과 실체적 경합범: 하나의 모조품 판매에 상표법+형법 별도 적용, 양형 합산 가능\n"
        "  ③ 관세법 제269조 제2항 (밀수입죄: 5년/관세액10배) + 제178조 (특허취소) + 제235조 (지식재산권보호)\n"
        ...
        "  ⑥ 소비자기본법 제4조(권리) + 제19조(사업자 책무: 불만·피해 신속처리, 위반시 시정명령) + 제46조(위해방지)\n"
        "     - 집단분쟁조정·소비자단체소송 가능성 반드시 언급\n"
        "  ⑦ 면세점 특수 가중요소\n"
...
def build_system_claude_v3(gatekeeper_text, gatekeeper_meta, internal_candidates=""):
    ...
    return (
        ...
        "형량/벌금/제재 수치는 절대 숫자로 직접 기재하지 마라.\n"
        "반드시 {{법령약칭_조문번호_내용}} 형태의 Placeholder로만 작성하라.\n"
        "예: {{상표법_제230조_7년이하징역또는1억원이하벌금}}, {{형법_제347조_20년이하징역또는5천만원이하벌금}}\n"
        "시스템이 Placeholder를 정확한 형량으로 자동 치환한다. Placeholder 없이 '7년 이하', '20년 이하' 등 숫자를 직접 적는 것은 금지.\n"
        ...
        "① 상표법: {{상표법_제230조_형량}} + {{상표법_제235조_양벌규정}} + {{상표법_제236조_필요적몰수}}\n"
        "   고의요건: 면세점=공식유통채널→'몰랐다'항변불가. 징벌적배상: 2025.7.22 시행, 종전3배→5배.\n"
        "② 형법: {{형법_제347조_형량}}. 2025.12.23개정 종전10년/2천만→현행(2배상향). 상표법과 실체적경합(양형합산).\n"
        "③ 관세법: {{관세법_제269조제2항_형량}} ⚠️반드시 '제2항'. 제3항과 혼동금지.\n"
        "   {{관세법_제235조_지식재산권보호}}. {{관세법_제178조_특허취소}}. 특가법제6조.\n"
        "④ 부정경쟁방지법: {{부정경쟁방지법_제18조제3항제1호_형량}}. 징벌적배상5배(2024개정).\n"
        "⑤ 보세판매장고시(관세청고시 제2023-9호): {{보세판매장고시_영업정지_6개월}}+{{보세판매장고시_특허취소}}+{{보세판매장고시_과징금}}.\n"
        "⑥ 소비자기본법: 제4조+제19조(사업자책무)+{{소비자기본법_제46조_위해방지}}. 집단분쟁·단체소송.\n"
        "⑦ 면세점특수가중: 국가특허→높은주의의무, 정품보증기대, 외국인국제파급.\n"
        "위험도기준: 🔴=형사처벌/특허취소 직결 🟡=행정제재/민사배상.\n"
        "████████████████████████████████████████████████████████\n\n"
    ...
    if model_choice == "claude":
        ...
        if pre_json:
            ...
            placeholder_db = {
                ...
                "상표법_제230조_형량": "7년 이하의 징역 또는 1억원 이하의 벌금",
                "상표법_제235조_3억원이하벌금": "3억원 이하의 벌금",
                "상표법_제235조_양벌규정": "법인에 대해 3억원 이하의 벌금(양벌규정)",
                ...
                "형법_제347조_형량": "20년 이하의 징역 또는 5천만원 이하의 벌금",
                # 관세법
                ...
                "관세법_제269조제2항_형량": "5년 이하의 징역 또는 관세액의 10배와 물품원가 중 높은 금액 이하의 벌금",
                "관세법_제269조_형량": "5년 이하의 징역 또는 관세액의 10배와 물품원가 중 높은 금액 이하의 벌금",
                "관세법_제235조_수출입원천금지": "수출입이 원천 금지",
                "관세법_제235조_지식재산권보호": "상표권 침해물품의 수출입이 원천 금지",
                "관세법_제178조_특허취소": "보세판매장 특허 취소",
                # 부정경쟁방지법
                "부정경쟁방지법_제18조제3항제1호_3년이하징역또는3천만원이하벌금": "3년 이하의 징역 또는 3천만원 이하의 벌금",
                "부정경쟁방지법_제18조제3항제1호_형량": "3년 이하의 징역 또는 3천만원 이하의 벌금",
                ...
                "보세판매장고시_과징금": "수입액에 비례한 과징금",
                # 소비자기본법
                "소비자기본법_제46조_수거파기등시정조치": "수거·파기·환급 등 시정조치",
                "소비자기본법_제46조_위해방지": "수거·파기·환급 등 위해방지 조치",
...
KNOWN_SARYU_NAMES = [
    # 기존 (사규 모음.docx 내부 챕터일 수 있어 유지)
    "MD 협력회사 입점 및 퇴점 지침",
    "직매입거래 기본계약서",
    "특정매입거래 기본계약서",
    "임대차계약서",
    "사내규정집",
    # 실제 docs 테이블 등록 label (Supabase 9건 기준)
    "사규: 사규 모음.docx",
    "사규 모음.docx",
    "계약서(특약매입): 특약매입.docx",
    "특약매입.docx",
                ...
                if st.button("🧪 MCP 연결 디버그", use_container_width=True, key="mcp_debug"):
                    with st.spinner("korean-law-mcp 테스트 중..."):
                        try:
                            ...
                            try:
                                ep = _req.get("https://korean-law-mcp.fly.dev/mcp", timeout=10)
                            ...
                            try:
                                ...
                                st.success(f"✅ MCP 연결 성공!")
                                st.caption(text[:300])
                            except Exception as e1:
                                err1 = str(e1)
                                st.error(f"❌ MCP 실패 — 에러 전문:")
                                st.code(err1[:1000], language="text")
                                
                                # 에러 유형 진단
                                if "input_schema" in err1:
                                    st.warning("💡 진단: MCP 서버의 도구 스키마가 Anthropic API 검증을 통과하지 못함. MCP 서버 측 문제.")
                                elif "authentication" in err1.lower() or "authorization" in err1.lower():
                                    st.warning("💡 진단: 인증 필요. MCP 서버에 API 키가 필요할 수 있음.")
                                elif "timeout" in err1.lower():
                                    st.warning("💡 진단: MCP 서버 응답 타임아웃. Fly.io 서버가 sleep 상태일 수 있음.")
                                elif "connect" in err1.lower():
                                    st.warning("💡 진단: MCP 서버에 연결 불가. 서버가 다운되었을 수 있음.")
                                else:
                                    st.warning("💡 에러 전문을 Claude에게 보여주세요.")
                        except Exception as e_all:
                            st.error(f"❌ 초기화 실패: {str(e_all)[:500]}")
            with col_api2:
                if st.button("🔄 법령 DB 업데이트", use_container_width=True, key="update_laws"):
                    with st.spinner("법령 DB 업데이트 중... (5~10분 소요, 창을 닫지 마세요)"):
                        try:
                            import update_laws
                            import importlib
