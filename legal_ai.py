...
def build_system(docs):
    ...
    return (
        ...
        "4. 결론의 근거로 반드시 **위반/부합하는 기준 문서(예: 사규 N조, 표준계약서 N조) 및 관련 법률명 제N조를 명시**하세요.\n"
        "5. 텍스트 강조 색상 규칙 (반드시 HTML 태그 사용):\n"
        "   - 검토 대상 텍스트 중 법률/사규 위반 소지가 있거나 당사에 불리한 독소조항 등 **위험하고 강조해야 할 텍스트는 반드시 `<span style='color:#FF3B30; font-weight:bold;'>텍스트</span>` 형태로 작성**하여 직관적인 빨간색으로 출력되게 하세요.\n"
        "   - 검토 대상 텍스트 중 법적/사규에 부합하거나 수용 가능한 긍정적 사항 등 **안전하고 강조해야 할 텍스트는 반드시 `<span style='color:#007AFF; font-weight:bold;'>텍스트</span>` 형태로 작성**하여 파란색으로 출력되게 하세요.\n"
        "6. 문서 교차 비교(V1 vs V2) 요청 시, 변경/추가/삭제된 텍스트를 명확히 대조(Diff)하고 협력사의 의도와 당사 리스크를 파악하세요.\n"
        "7. 💡 답변의 마지막에는 반드시 **[최종 AI변호사 검토 의견 및 실무 가이드]** 섹션을 추가하여 ① 최종 결론, ② MD Action Plan(수정 대안, 협상 논리)을 명확히 제시하세요."
    )
...
def main():
    ...
    for msg in st.session_state.messages:
        with st.chat_message(msg["role"], avatar="⚖" if msg["role"] == "assistant" else "👤"):
            # HTML 태그 렌더링을 위해 unsafe_allow_html 옵션 추가
