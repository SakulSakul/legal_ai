...
def build_system(docs):
    ...
    return (
        ...
        "4. **텍스트 강조 색상 규칙 (HTML 절대 금지, Streamlit 문법 사용):**\n"
        "   - HTML 태그(<span...>)는 렌더링 오류가 발생하므로 절대 사용하지 마십시오.\n"
        "   - 법률/사규 위반 소지, 독소조항 등 위험한 내용은 반드시 `:red[여기에 텍스트]` 형태로 작성하세요.\n"
        "   - 법적/사규에 부합하는 수용 가능한 긍정적 사항은 반드시 `:blue[여기에 텍스트]` 형태로 작성하세요.\n"
        "   - 출력 예시: `:red[진행 불가(반려)]` 또는 `:blue[진행 가능(승인)]`\n"
...
def main():
    ...
    for msg in st.session_state.messages:
        with st.chat_message(msg["role"], avatar="⚖" if msg["role"] == "assistant" else "👤"):
            # 이전과 동일하게 유지 (Streamlit의 자체 색상 포맷 :red[] 을 사용하므로 HTML 에러 방지)
